...
class VentanaLogin:
    ...
    def _login(self):
        """Valida usuario y contraseña y abre la ventana principal si son correctos."""
        usuario = self.entry_usuario.get().strip()
        contrasena = self.entry_contrasena.get().strip()

        # Llamar a la función que autentica al usuario desde la base de datos
        user_data = autenticar_usuario(usuario, contrasena)
        if user_data:
            # Si las credenciales son correctas, abrir ventana principal y cerrar login
            user_id, nombre, rol = user_data
            self.ventana.destroy()
            VentanaPrincipal(usuario=nombre, rol=rol)
        else:
    ...
    def _abrir_registro(self):
        ...
        combo_rol.set("docente")  # Valor por defecto
